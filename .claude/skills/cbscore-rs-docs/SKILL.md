---
name: cbscore-rs-docs
description: >
  Use when creating, naming, or placing design documents, plans, or
  review documents for the cbscore or cbscore-rs packages in the cbs.git
  repository. Covers directory routing, file name format, sequence
  numbering, and timestamp conventions for cbscore-rs/docs/.
---

# cbscore-rs-docs — Document Storage & Naming

## Directory Structure

All documentation lives under `cbscore-rs/docs/` (repo root–relative):
...
- `cbscore` package → `cbscore-rs/docs/cbscpre/`
- All cbscore-rs workspace packages → `cbscore-rs/docs/cbscore-rs/`
...
- To find the next `seq`: scan all files under
  `cbscore-rs/docs/<package>/design/`, extract the highest numeric prefix,
...
```
# Design document (seq 009, cbscore-rs package):
cbscore-rs/docs/cbscore-rs/design/
  009-20260320T0750-dev-oauth-bypass.md

# Single plan for that design (same seq, no sub):
cbscore-rs/docs/cbscore-rs/plans/
...
cbscore-rs/docs/cbscore-rs/reviews/
...
# cbscore design — cbscore has its own independent seq (next: 007):
cbscore-rs/docs/cbscore/design/
