...
///
/// Surface mirrors [`crate::config::VersionedConfig`] but uses the
/// snake-case tag `schema_version` (descriptors are JSON with
/// snake-case keys per design 002 §Wire-Format Versioning).
///
/// # Examples
///
/// ```
...
impl VersionedVersionDescriptor {
    ...
    #[must_use]
    pub const fn new(desc: VersionDescriptor) -> Self {
        Self::V1(desc)
    }
