...
| Subprocess + redaction      | `tokio::process` + custom `SecureArg` trait         | `tokio` 1 (re-use)                                              | Ports `utils/__init__.py` (`async_run_cmd`, `SecureArg`, `_sanitize_cmd`)                                                                                                             |
...
| git                         | `tokio::process` + `git` binary                     | `tokio` 1 (re-use); `git` binary `>= 2.23`                      | Matches Python: `utils/git.py` is 401 LoC of subprocess wrappers. Floor `git 2.23` (Aug 2019) is set by `git switch` and `git branch --show-current`, used by `cbscommon/git/cmds.py` |
