...
1. Who creates the sender — dispatch (Commit 8a) or first `build_output` (Commit 9)? If at first output, there's a race where SSE connects before output arrives.
2. Who drops the sender on completion — `build_finished` handler, startup recovery (Commit 12), or shutdown (Commit 13)?
