...
The updated commit includes `cargo fmt` reformatting of surrounding code in
`main.rs` and `routes/auth.rs`. These are formatting-only changes (`VERSION`
const, `setup_tracing` arguments, `GovernorError` tuple, `format!` calls, method
chain line-breaks). They increase the diff size (208/148 vs 192/112) but don't
