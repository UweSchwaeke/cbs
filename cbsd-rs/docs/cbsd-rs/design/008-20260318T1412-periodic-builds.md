...
`periodic:manage:any`. If the request includes a `descriptor`
...
prevents a user with `periodic:manage:{own,any}` but limited build scopes
from redirecting a task to a channel they don't have access to.
...
All four are added to `KNOWN_CAPS` in `permissions.rs`. The `admin`
role's wildcard `*` already covers them. Only admins get periodic build
capabilities at this point.

`periodic:create` and `periodic:manage:{own,any}` are independent
capabilities — neither is a superset of the other. Creating a task
requires `periodic:create` + `builds:create` (with scopes). Managing
existing tasks requires `periodic:manage:own` (with `created_by ==
user.email`) or `periodic:manage:any`, plus `builds:create` with scopes
if updating the descriptor.
