...
Create the `cbsd-rs/` workspace directory at the repository root and populate it
with a `CLAUDE.md` that provides implementation context for all future Claude
sessions working on the Rust codebase.
...
2. **Skill references** — instructions to always consult:
   - `/rust-2024` for all Rust code (2024 edition, axum, tokio, serde, sqlx,
     tracing, error handling, async patterns)
...
5. **Key architecture pointers:**
   - Design documents at `cbsd-rs/docs/cbsd-rs/design/` (authoritative)
   - Implementation plans at `cbsd-rs/docs/cbsd-rs/plans/` (update progress
     after each commit)
   - SQLite with sqlx (compile-time checked queries, offline cache in `.sqlx/`,
     `SQLX_OFFLINE=true` for CI)
   - Workspace layout: `cbsd-proto` (shared types, no IO), `cbsd-server` (axum
     REST + WS), `cbsd-worker` (WS client + subprocess)
...
6. **Correctness invariants** (easy to miss, document prominently):
   ...
   - `tower-sessions-sqlx-store` `.migrate().await` called after sqlx migrations
     but before router is built (creates `tower_sessions` table)
...
7. **sqlx offline query cache:** `.sqlx/` directory lives at workspace root
   ...
   `cargo sqlx prepare --workspace` and include `.sqlx/` in the commit.** CI
   builds use `SQLX_OFFLINE=true`.

8. **Implementation guidance:**
   - The design docs are authoritative — if code and design disagree, fix the
     code
   ...
   - Consult `cbsdcore/src/cbsdcore/versions.py` for Python BuildDescriptor when
     porting types
   - Consult `cbsd/cbslib/core/permissions.py` for Python permission model as
     reference

**Testable:** `cbsd-rs/` directory exists with `CLAUDE.md`. No Cargo files yet
